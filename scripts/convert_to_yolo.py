...
def setup_yolo_structure():
    ...
    print(f"[OK] Created YOLO directory structure at {YOLO_DIR}")
...
def process_angelina():
    ...
    for img_path in sorted(ANGELINA_DIR.rglob("*.labeled.jpg")) + sorted(
        ANGELINA_DIR.rglob("*.labeled.png")
    ):
        # Skip git directory
        ...
        try:
            ...
            print(f"[WARN] Skipping {img_path.name} (can't read annotation)")
            continue
    ...
    print(
        f"   [OK] Converted {image_count} images with {annotation_count} total annotations"
    )
    return image_count
...
def process_roboflow():
    ...
    if not ROBOFLOW_DIR.exists() or not list(ROBOFLOW_DIR.glob("*")):
        print("   [WARN] Roboflow dataset not found (manual download required)")
        print(
            "   Instructions: Download from Roboflow, extract to data/training/roboflow_v2/"
        )
        return 0

    # Count images if they exist
    images = list(ROBOFLOW_DIR.rglob("*.jpg")) + list(ROBOFLOW_DIR.rglob("*.png"))
    print(f"   Found {len(images)} images in Roboflow directory")
    return len(images)
...
def create_train_val_test_split(total_images, train_ratio=0.7, val_ratio=0.15):
    ...
    test_count = n - train_count - val_count  # noqa: F841
    ...
    print(
        f"   [OK] Train: {len(train_images)}, Val: {len(val_images)}, Test: {len(test_images)}"
    )
...
def main():
    print("[START] Converting datasets to YOLO format...\n")
    ...
    if total == 0:
        print("\n[ERROR] No images processed. Check your datasets.")
        return
    ...
    print(f"\n[OK] Training data ready at {YOLO_DIR}")
    print(f"   Total images: {total}")
    print(
        f"   Run: yolo detect train model=yolov8n.pt data={YOLO_DIR}/dataset.yaml epochs=50"
    )
